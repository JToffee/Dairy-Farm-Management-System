...
export default {
  ...
  computed: {
    ...
    prepaidAmount() {
      return Intl.NumberFormat("en-GB").format(sumSale(this.durationCatData("prepaid")));
    },
    prepaidDiff() {
  ...
  methods: {
    ...
    differenceByCat(category = null, data) {
      ...
      if (this.days === 0) {
        ...
        prevDate = new Date(new Date().setHours(0, 0, 0, 0)).getTime() - 1 * TOMILLISECS;
    ...
    differenceOfSum() {
      ...
      if (this.days === 0) {
        ...
        currData = sumSale(this.data.filter((item) => item.date === this.setStartDate()));
        ...
        prevData = sumSale(
          this.data.filter((item) => item.date <= this.startDate && item.date >= prevDate)
        );
        console.log(currData, prevData);
      }
